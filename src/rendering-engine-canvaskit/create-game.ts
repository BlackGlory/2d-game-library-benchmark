...
export function createGame(canvas: HTMLCanvasElement): GameLoop<number> {
  ...
  offscreenCanvas.width = SCREEN_WIDTH_PIXELS
  offscreenCanvas.height = SCREEN_HEIGHT_PIXELS
  const surface = CanvasKit.MakeWebGLCanvasSurface(offscreenCanvas)!
  ...
  function renderingSystem(alpha: number): void {
    {
      ...
      for (const entityId of queryBox.findAllEntityIds()) {
        ...
        paint.setColor(CanvasKit.parseColorString(COLORS[color]))
